...
@Autonomous(name = "BlueRightClose", group = "BLUE")
    ...
    public void runAutonoumousMode() {
        ...
        switch (propPosition) { //UPDATE THESE POSITIONS
            case LEFT:
                ...
                wallPose = new Pose2d(15, 2, Math.toRadians(0));
                purplePixelPose = new Pose2d(30, 11, Math.toRadians(60.0));
                break;
            case CENTER:
                telemetry.addData("Center Position", "Center");
                wallPose = new Pose2d(6, 2, Math.toRadians(0));
                purplePixelPose = new Pose2d(26, 5, Math.toRadians(0.0));
                break;
            case RIGHT:
                wallPose = new Pose2d(5, 2, Math.toRadians(0));
                telemetry.addData("Right Position", "Right");
                purplePixelPose = new Pose2d(22,-8, Math.toRadians(0));
                break;

        }
        Pose2d firstparkingPose = new Pose2d(7,25 , Math.toRadians(0));
        Pose2d parkingPose = new Pose2d(7, 70, Math.toRadians(0)); //UPDATE
